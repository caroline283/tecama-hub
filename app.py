...
# --- 2. CSS v6.6 (FONTE GRANDE E BOTÕES) ---
st.markdown("""
    <style>
...
def norm(t):
    """Limpeza profunda para PDF e CSV"""
...
with st.sidebar:
    ...
    st.caption("Tecama Hub Industrial v9.4")
    ...
    with aba_conv:
                ...
                with pd.ExcelWriter(output, engine="openpyxl") as writer:
                    ...
                    for dp, g in df.sort_values(by="DES_PAI").groupby("DES_PAI", sort=False):
                        ...
                        for _, r in g.iterrows():
                            for i, c_n in enumerate(["QUANT","COMP","LARG","MATERIAL","COR","DESCPECA","DES_PAI","CORTE","FITA","USINAGEM","PESO_UNIT","PESO_TOTAL"], 1):
                                cell = ws.cell(row=curr, column=i, value=r.get(c_n, ""))
        ...
        if up_excel_f2:
            if st.button("🚀 Gerar CSV para Corte Certo"):
                nome_f2 = up_excel_f2.name.replace(".xlsx", "").upper()
                ...
                st.download_button("📥 Baixar CSV Corte Certo", csv_out, f"CORTE_CERTO_{nome_f2}.csv", "text/csv")
